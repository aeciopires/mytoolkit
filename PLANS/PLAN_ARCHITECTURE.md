...
| `internal/web/templates/partials/tool-panel.html` + `internal/web/static/js/tool-common.js` | Shared `{{define "tool-panel"}}` markup block (input/output textareas, button row) and shared vanilla-JS helpers (debounced `fetch()`, copy-to-clipboard, blob download, error-banner rendering), reused by every `tools/<tool>.html` page. A `data-client-side` attribute on `.tool-panel` opts a page out of the shared fetch-based `run()` wiring (copy/reset/download wiring still applies) for tools whose web page must never call the server — see `PLAN_JSON_TOON_CONVERTER.md`. | All 12 tool pages; each page template supplies only its tool-specific extra controls (e.g. Password Generator's checkboxes, Case Converter's mode buttons) around the shared partial. |
...
Tool slugs (kebab-case, shared with CLI subcommand names): `json-tree`, `json-format`, `yaml-format`, `password-gen`, `jwt`, `qrcode`, `text-count`, `url-encode`, `hash-gen`, `base64`, `case-convert`, `json-toon`.
...
- `README.md`'s [Documentation](#documentation-structure) table has one row per feature with working links to all 36 `docs/api|cli|testing/<slug>.md` files plus `docs/environment-variables.md`; no dead or missing links.
