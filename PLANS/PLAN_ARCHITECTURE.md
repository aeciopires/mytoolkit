...
**API documentation**: `GET /swagger/index.html` serves an interactive Swagger UI (swaggo/swag + swaggo/http-swagger) covering every JSON REST endpoint — all `/api/v1/tools/<slug>` routes, `GET /api/v1/tools`, `GET /api/v1/metrics/ranking`, `/healthz`, `/readyz`. `/metrics` (Prometheus text format, not JSON) is deliberately excluded — it isn't a "REST API" endpoint in the CRUD/JSON sense the rest of this section describes, and Prometheus's exposition format doesn't fit an OpenAPI schema. The spec is generated from `@`-comment annotations above each handler function (`make swagger-gen`; see `.skills/swagger/SKILL.md`) — chi's actual route table has one templated `/api/v1/tools/{slug}` route, but the generated Swagger spec documents each tool as its own literal path (`/api/v1/tools/base64`, `/api/v1/tools/json-format`, ...) for per-tool request/response detail, matching how `docs/api/<slug>.md` already documents them individually.
...
- **Swagger/OpenAPI documentation**: at the user's explicit request to use `swaggo/swag`. Added general API annotations above `main()` and per-endpoint annotations above each REST handler, generated into `app/docs/` via `make swagger-gen` (a pinned `go run github.com/swaggo/swag/cmd/swag@v1.16.6`, no separate install required), and mounted a Swagger UI at `GET /swagger/index.html` via `swaggo/http-swagger`. The main implementation obstacle: `swag` only resolves a cross-package type reference in an annotation if the annotated file actually imports that package — none of the 9 tools wired via the generic `handlers.Wrap(...)` had a named handler function to attach a comment to, and none of the 15 `internal/cli/<tool>.go` files import `internal/httpapi` (where a first draft of the doc-only response DTOs was placed). Fixed by (1) giving each `handlers.Wrap`-based tool a small named wrapper function (e.g. `func base64Handler() http.HandlerFunc { return handlers.Wrap(...) }`) purely to carry the annotation, and (2) moving the shared doc-only DTOs (`ToolSuccessResponse`, `ToolErrorResponse`, `ToolMeta`) into `internal/cli` itself, since that package is what every annotation actually lives in. Verified end-to-end: `go build`/`vet`/`test` (including a new `internal/httpapi/swagger_test.go` regression test asserting every `registry.All()` slug has a corresponding Swagger path, so a future tool added without annotations fails a test instead of silently missing from the docs), a real running binary's `/swagger/doc.json` and `/swagger/index.html`, and a Playwright-driven "Try it out" execution against the live `/api/v1/tools/base64` endpoint returning a real 200 response. `/metrics` is deliberately not documented in Swagger (Prometheus exposition format, not JSON/OpenAPI-shaped).
- **Helm chart `appVersion` synced from `VERSION`**: at the user's explicit request. `Chart.yaml`'s `appVersion` field was previously hand-maintained in lockstep with the root `VERSION` file (see [Versioning](#versioning) in `CLAUDE.md`) — nothing enforced that they actually stayed in sync. Added a `helm-set-appversion` Makefile target (`sed`-rewrites `appVersion: "..."` in `$(CHART_DIR)/Chart.yaml` to `$(VERSION)`, using the portable `-i.bak` + `rm` idiom rather than GNU-only `sed -i` so it also works on BSD/macOS `sed`) and made it a prerequisite of `helm-docs`, so every chart-docs regeneration re-syncs `appVersion` first — a hand-edited `appVersion` would just get overwritten on the next `make helm-docs`, same as `helm/mytoolkit/README.md` itself. The chart's own `version:` field (semver of the chart, independent of the app version by Helm convention) is intentionally untouched. Verified by temporarily setting `VERSION` to a different value, confirming `helm-set-appversion` rewrote `Chart.yaml` correctly, then restoring both files and confirming `git diff` was clean; also ran the real `make helm-docs` → `make helm-lint` sequence end-to-end.
